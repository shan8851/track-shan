"use client";

import { ArrowRight, Plus } from "lucide-react";
import Link from "next/link";
import { useState } from "react";

import { Button } from "@/components/ui/button";
import { DeleteConfirmDialog } from "@/components/shared/deleteConfirmDialog";
import { PaginationControls } from "@/components/shared/paginationControls";
import { ExerciseFormModal } from "@/components/dashboard/exerciseFormModal";
import { ExerciseStats } from "@/components/dashboard/exerciseStats";
import { ExerciseTable } from "@/components/dashboard/exerciseTable";
import { useDeleteExercise, useExerciseList } from "@/hooks/useExercise";
import { DEFAULT_PAGE_SIZE } from "@/lib/constants";
import type { ExerciseEntry } from "@/types/exercise";

export const ExerciseSection = () => {
  const [page, setPage] = useState(1);
  const [formOpen, setFormOpen] = useState(false);
  const [editEntry, setEditEntry] = useState<ExerciseEntry | null>(null);
  const [deleteEntry, setDeleteEntry] = useState<ExerciseEntry | null>(null);

  const { data, isLoading } = useExerciseList(page, DEFAULT_PAGE_SIZE);
  const deleteMutation = useDeleteExercise();

  const handleEdit = (entry: ExerciseEntry) => {
    setEditEntry(entry);
    setFormOpen(true);
  };

  const handleDelete = (entry: ExerciseEntry) => {
    setDeleteEntry(entry);
  };

  const confirmDelete = async () => {
    if (!deleteEntry) return;
    try {
      await deleteMutation.mutateAsync(deleteEntry.id);
      setDeleteEntry(null);
    } catch {
      // handled by TanStack Query
    }
  };

  const handleFormClose = (open: boolean) => {
    setFormOpen(open);
    if (!open) setEditEntry(null);
  };

  return (
    <section className="space-y-4" data-testid="exercise-section">
      <div className="flex items-center justify-between">
        <Link href="/exercise" className="group flex items-center gap-2 hover:opacity-80 transition-opacity">
          <h2 className="text-lg font-bold text-terminal">&gt; exercise</h2>
          <ArrowRight className="h-4 w-4 text-terminal opacity-0 group-hover:opacity-100 transition-opacity" />
        </Link>
        <Button size="sm" onClick={() => setFormOpen(true)}>
          <Plus className="h-4 w-4 mr-1" />
          New Session
        </Button>
      </div>

      <ExerciseStats />

      {isLoading ? (
        <div className="border border-border p-8 text-center text-muted-foreground animate-pulse">
          Loading...
        </div>
      ) : (
        <>
          <ExerciseTable
            entries={data?.data ?? []}
            onEdit={handleEdit}
            onDelete={handleDelete}
          />
          {data?.pagination && (
            <PaginationControls
              page={data.pagination.page}
              totalPages={data.pagination.totalPages}
              onPageChange={setPage}
            />
          )}
        </>
      )}

      <ExerciseFormModal
        open={formOpen}
        onOpenChange={handleFormClose}
        editEntry={editEntry}
      />

      <DeleteConfirmDialog
        open={!!deleteEntry}
        onOpenChange={(open) => !open && setDeleteEntry(null)}
        onConfirm={confirmDelete}
        title="Delete exercise entry"
        description={`Delete the session from ${deleteEntry?.date ?? ""}?`}
      />
    </section>
  );
};
