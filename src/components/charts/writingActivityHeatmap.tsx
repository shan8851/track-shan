...
export const WritingActivityHeatmap = () => {
  ...
  const { grid, dayMap, intensityMap, totalCols } = useMemo(() => {
    ...
    const writingDayMap = aggregateWritingByDay(entries ?? []);
    const writingIntensityMap = computeIntensityLevels(writingDayMap);
    const maxCol = yearGrid.reduce((max, day) => Math.max(max, day.col), 0);

    return {
      grid: yearGrid,
      dayMap: writingDayMap,
      intensityMap: writingIntensityMap,
      totalCols: maxCol + 1,
    };
  }, [entries]);
        ...
        <svg width={svgWidth} height={svgHeight} className="block">
          {monthLabels.map((month) => (
            <text
              ...
              x={month.col * (CELL_SIZE + CELL_GAP) + CELL_GAP}
              y={12}
          ...
          {grid.map((day) => {
            ...
            const entryCount = dayMap.get(day.date) ?? 0;
            return (
              <rect
                key={day.date}
              ...
              >
                <title>
                  {formatDate(day.date)}: {entryCount > 0 ? `${entryCount} entries` : "No writing"}
                </title>
              </rect>
            );
          })}
